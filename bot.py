...
callback_data=f”cv_{safe_key(uname)}”
)])
...
query = update.callback_query
await query.answer()
key = query.data.replace(“cv_”, “”)
data = load()
uname = find_by_key(data, key)
if not uname:
await query.edit_message_text(“Клиент не найден.”)
return
c = data[“clients”][uname]
st = client_status_icon(c)
sub = get_active_sub(c)
...
k = safe_key(uname)
kb = [
[InlineKeyboardButton(“➕ Добавить абонемент”, callback_data=f”addsub_{k}”)],
[InlineKeyboardButton(“✏️ Редактировать данные”, callback_data=f”ec_{k}”)],
...
kb.append([InlineKeyboardButton(“◀️ Назад”, callback_data=f”cv_{key}”)])
...
return EDIT_CLIENT_VALUE
save(data)
kb = [[InlineKeyboardButton(“◀️ К клиенту”, callback_data=f”cv_{safe_key(uname)}”)]]
...
query = update.callback_query
await query.answer()
key = query.data.replace(“addsub_”, “”)
data = load()
uname = find_by_key(data, key)
if not uname:
await query.edit_message_text(“Клиент не найден.”)
...
query = update.callback_query
await query.answer()
raw = query.data.replace(“training_”, “”)
day_key = raw.split(”*”)[0] if “*” in raw else raw
data = load()
...
if status == “signed”:
kb.append([
InlineKeyboardButton(f”✅ {name}”, callback_data=f”att_y_{day_key}*{sk}”),
InlineKeyboardButton(“❌”, callback_data=f”att_n*{day_key}*{sk}”),
InlineKeyboardButton(“🚫”, callback_data=f”att_c*{day_key}*{sk}”),
])
kb.append([InlineKeyboardButton(“✏️ Редактировать тренировку”, callback_data=f”edit_training*{day_key}”)])
...
kb.append([InlineKeyboardButton(
f”{icon} {name} @{uname}”,
callback_data=f”es_{day_key}*{safe_key(uname)}”
)])
kb.append([InlineKeyboardButton(“◀️ Назад”, callback_data=f”training*{day_key}”)])
await query.edit_message_text(
...
query = update.callback_query
await query.answer()
parts = query.data.replace(“es_”, “”).split(”*”)
day_key = parts[0]
uname_key = parts[1]
data = load()
uname = find_by_key(data, uname_key)
if not uname:
await query.edit_message_text(“Клиент не найден.”)
return
...
[InlineKeyboardButton(“◀️ Назад”, callback_data=f”edit_training*{day_key}”)],
]
await query.edit_message_text(
...
last_sub_idx = len(client[“subscriptions”]) - 1
...
[InlineKeyboardButton(“◀️ Назад”, callback_data=f”training*{day_key}”)],
]
await query.edit_message_text(
...
kb.append([InlineKeyboardButton(
f”{c[‘name’]} @{uname}”,
callback_data=f”dosignup_{day_key}*{safe_key(uname)}”
)])
kb.append([InlineKeyboardButton(“◀️ Назад”, callback_data=f”training*{day_key}”)])
...
query = update.callback_query
await query.answer()
parts = query.data.replace(“dosignup_”, “”).split(”*”)
day_key = parts[0]
uname_key = parts[1]
data = load()
uname = find_by_key(data, uname_key)
...
edit_client_conv = ConversationHandler(
    entry_points=[CallbackQueryHandler(edit_client_menu, pattern="^ec_")],
    states={
...
app.add_handler(CallbackQueryHandler(view_client, pattern="^cv_"))
...
app.add_handler(CallbackQueryHandler(attend_handler, pattern="^att_"))
app.add_handler(CallbackQueryHandler(edit_training, pattern="^edit_training_"))
app.add_handler(CallbackQueryHandler(edit_status_choose, pattern="^es_"))
